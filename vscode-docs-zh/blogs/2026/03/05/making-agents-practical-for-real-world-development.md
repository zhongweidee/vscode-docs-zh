...
Order: 128
TOCTitle: 面向实际开发的智能体
PageTitle: "让智能体在实际开发中切实可用"
MetaDescription: "探索 VS Code 1.110 中的智能体编排、可扩展性和连续性：生命周期钩子、智能体技能、会话记忆和集成浏览器工具。"
...
2026 年 3 月 5 日，VS Code 团队，[@code](https://x.com/code)

智能体正在承担更复杂、更耗时的开发任务。

通过 [2026 年 2 月版本（1.110）](http://aka.ms/VSCode/110)，我们在 Visual Studio Code 中让这些工作流更加实用，让您能更好地控制智能体的行为方式、集成到工具中的方式，以及跨会话保留项目上下文的方式。
...
<video src="embedded-terminal-streaming.mp4" title="展示长时间运行命令时终端输出自动展开的视频。" autoplay muted controls></video>
...
![显示上下文窗口控件和压缩选项的截图。](context-compaction.png)
...
在我们的应用中，当引入新的样式指导以金色点缀和闪光效果增强英雄卡片时，智能体会重新审视现有 CSS 并[在不重新开始会话的情况下](https://code.visualstudio.com/docs/chat/chat-sessions#_send-messages-while-a-request-is-running)继续实现。
...
在下面的演示中，[`/fork`](https://code.visualstudio.com/docs/chat/chat-sessions#_fork-a-chat-session) 创建了一个并行线程，在其中探索了更简约的设计方向，而不影响原始讨论。

<video src="fork-a-conversation.mp4" title="展示在 VS Code 中将对话分叉为并行线程的视频。" autoplay muted controls></video>

### 通过钩子实现自动化

团队经常依赖约定、验证或自动检查来保持一致性。

[钩子](https://code.visualstudio.com/docs/agent-customization/hooks)在关键生命周期事件中确定性执行，允许团队强制执行策略并设置护栏，使智能体驱动的更改符合项目标准，而不是依赖重复的提示。
...
<video src="automate-with-hooks.mp4" title="展示在 VS Code 中会话退出时执行停止钩子，检测未提交更改并自动提交和推送的视频。" autoplay muted controls></video>
...
无需每次都重新编写指令，您可以使用斜杠命令直接从聊天中调用智能体技能。[技能](https://code.visualstudio.com/docs/agent-customization/agent-skills)可以来自内置功能、扩展或项目特定的工具。
...
<video src="use-skills-on-demand.mp4" title="展示前端设计技能实现 UI 组件并验证结果的视频。" autoplay muted controls></video>
...
<video src="use-integrated-browser.mp4" title="展示在 VS Code 集成浏览器中进行 UI 更改的浏览器验证的视频。" autoplay muted controls></video>
...
![Copilot CLI 界面的截图，显示 VS Code 自动连接设置和已选择的工作区。](copilot-cli.png)

## VS Code 中智能体的下一步

智能体正在成为日常开发中自然的一部分。您不必围绕它们调整工作流——它们应该适应您的构建方式。

通过 [2026 年 2 月版本（1.110）](http://aka.ms/VSCode/110)，VS Code 让您更好地控制智能体的行为方式。它们更自然地融入您的工具，并跨会话携带上下文。

我们正在以公开透明的方式构建这一切。如果您有反馈、想法或遇到问题，请在 [VS Code 仓库](https://github.com/microsoft/vscode/issues)中发起讨论或提交问题，或通过社交媒体找到我们。我们很乐意听取您的意见。

想了解这些功能如何增强您的开发工作流吗？

**请加入我们于太平洋标准时间 3 月 19 日上午 8 点举行的 VS Code 版本发布直播。[开启通知](http://aka.ms/VSCode/Mar19)！**
