---
Order: 129
TOCTitle: VS Code 如何利用 AI 进行构建
PageTitle: "VS Code 如何利用 AI 进行构建"
MetaDescription: 了解 VS Code 如何在其自身开发工作流中使用 AI，包括 GitHub Copilot 代理模式、自动化测试和 AI 驱动的代码审查。
...
2026年3月13日，作者 [Pierce Boggan](https://github.com/pierceboggan)

我们每天都在使用 AI 来交付 VS Code。它让我们变得如此高效，以至于在坚持了十年每月发布之后，我们刚刚转向了每周发布。智能体是解锁这一切的关键，不仅用于编写代码，还贯穿于团队工作的方方面面。

为了开启[智能体会话日](https://youtube.com/live/tAezuMSJuFs)，我与 VS Code 团队的工程经理彭律坐在一起，深入探讨了 VS Code 团队如何在日常工作中实际使用 AI。不仅仅是实现功能（这部分不言自明），而是围绕构建功能的一切：分类、代码审查、发布说明、验证，以及在会议密集的时间表中保持高效。

<iframe width="560" height="315" src="https://www.youtube-nocookie.com/embed/ee-obY-4rqk?si=N-y6jlg15iBzoKxH" title="智能体会话日：VS Code 如何利用 AI 进行构建" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
![一张来自 @pierceboggan 在 X 上的帖子截图，内容为："你们告诉我们希望将 VS Code Insiders 中的功能更快地提供到 VS Code 稳定版。我们正在向每周稳定版发布迈进，以便将顶级功能更快地带给 VS Code。"](weekly-releases-x.png)

每周节奏并非为快而快。它是为了更快地将改进带给开发者。一个 bug 修复过去要等三周才能进入下一个稳定版本，现在几天就能交付。一个周一合并的功能，当周就能出现在开发者的编辑器里。那个*交付 > 学习 > 迭代*的反馈循环变得快得多。

## 我们从这一转变中学到的

我们的工作流和流程每天都在持续演进，因为我们边学边适应。但有一些关键经验始终成立：

1. **并行化自己。** 养成在切换上下文之前启动多个智能体会话的习惯。工作树、云端智能体、多个 VS Code 会话……全部用上。
2. **跳过中间产物。** 过去是会议记录 → 议题 → 规格说明 → 代码，现在实际上正在变成会议 → 智能体会话 → 代码 → PR。
3. **自动化那些随速度增长的开销。** 我们为议题分类、提交摘要、发布说明、代码审查构建了智能体驱动的流水线，全部使用 Copilot CLI、Copilot SDK 和 GitHub Actions。工程师仍然在这些工作流的另一端，但智能体帮助更快地将正确的事情呈现给正确的人。
4. **先投资约束框架，再追求速度。** 测试、黄金场景和代码审查关卡可以防止智能体驱动的速度变成智能体驱动的回归。
5. **所有权正在演变。** 当产品经理、其他领域的工程师、社区贡献者和智能体都可以为任何组件做出贡献时，传统的所有权模型需要适应变化。对结果的责任仍然落在工程师身上。
6. **在品味方面让人类保持在回路中。** 智能体检查正确性，人类评估愉悦感。

让我们更详细地看看这些经验在团队中的具体体现。

### 并行工作

Paul Graham 写过一篇著名的[文章](https://paulgraham.com/makersschedule.html)，讲述创造者时间表和管理者时间表从根本上是不兼容的。在最近之前，这基本上是正确的。但智能体正在改变这一局面。
...
让我给你一个真实的例子。彭律每天早上从更新 [VS Code Insiders](https://code.visualstudio.com/insiders) 开始。大多数日子，我们每天交付两次 Insiders 构建，以便早期获得我们正在开发的功能的反馈。然后他运行一个自定义智能体，通过 [Work IQ](https://learn.microsoft.com/microsoft-365-copilot/extensibility/workiq-overview) 获取他的会议安排，并生成他当前任务盘点的快照。

从那里开始，彭律决定什么需要他亲自关注，什么委托给智能体，以及为团队优先处理什么。智能体处理收集上下文的繁琐工作，这样他就可以直接切入有趣的问题。当他进入第一个电话会议时，任务已经在并行运行了。

![一个任务管理视图，显示优先级待办列表，分为两部分："自己做（人员/决策）"包含 4 项内容，包括为 VS Code Live 智能体会话日做准备、安排一对一会议、发送仓库链接和沟通退出期望；以及"开放代码任务（委托或自己做）"包含 3 项内容，包括后台智能体工作树改进、启动群聊以协调审查，以及讨论用于引导上下文的 GitHub 端点。每个项目包含状态说明和日期。](task-management.png)
...
![标题为"VS Code 仓库活动 1月1日至3月10日"的柱状图，显示使用公开 GitHub 数据的年同比对比。提交量从 2025 年的 2,339 增长到 2026 年的 5,104，增长 2.2 倍。关闭的议题从 2025 年的 2,916 增长到 2026 年的 8,402，增长 2.9 倍。2025 年的柱状图为灰色，2026 年的柱状图为蓝色。](repo-activity.png)
...
![一张来自 @pierceboggan 在 X 上的帖子截图，分享分叉功能即将进入 VS Code。](fork.png)
...
- **测试。** 全面的测试套件、单元测试、集成测试以及运行它们的基础设施，只是基本要求。除此之外，我们记录了**黄金场景**：核心用户流的预期行为规格。我们传统上在月度收尾周手动测试这些场景。我们现在将这些场景交给智能体作为自动化合并后验证来运行。我们还在探索使用这条流水线自动生成演示录制：一个 PR 落地，一个演示视频生成，这就成了更新日志或推文的内容。
...
我们真诚地好奇：你们团队的版本是什么样的？有没有我们遗漏的工作流？你们自动化了哪些我们还没想到的事情？请在 [VS Code 仓库](https://github.com/microsoft/vscode/issues)中给我们提一个议题，或者在 [X](https://x.com/code) 上找到我们——我们与你一起构建，你的反馈会塑造下一步的发展。

我们在[智能体会话日](https://aka.ms/VSCode/Feb19)上还有很多其他精彩的会议，如果你还没有看过，不妨去看看。
